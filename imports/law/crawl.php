...
class Crawler
{
    ...
    {
        if (!file_exists(__DIR__ . '/law-data/laws-category.csv')) {
            return array();
        }
    ...
    public function crawlLaw($category, $category2, $status, $title, $url)
    {
        ...
        if (!file_exists(__DIR__ . "/law-data/laws/{$law_id}")) {
            mkdir(__DIR__ . "/law-data/laws/{$law_id}");
        }
        ...
        foreach ($doc->getElementsByTagName('td') as $td_dom) {
            ...
            $content = $this->http($url);
            file_put_contents(__DIR__ . "/law-data/laws/{$law_id}/{$versions[0]}.html", $content);
            ...
            foreach ($law_doc->getElementsByTagName('img') as $img_dom) {
                foreach ($btn_map as $id => $name) {
                    if ($img_dom->getAttribute('src') == "/lglaw/images/{$id}.png"){
                        ...
                        $content = $this->http($url);
                        file_put_contents(__DIR__ . "/law-data/laws/{$law_id}/{$versions[0]}-{$name}.html", $content);
                        //$this->fetchRelate($content);
                        $types[] = $name;
    ...
    public static function addLawVersion($law_id, $title, $versions, $types)
    {
        ...
        if (file_exists(__DIR__ . '/law-data/laws-versions.csv')) {
        ...
        uksort($law_versions, function($k1, $k2) use ($law_versions) {
            list($id1, $k1) = explode('-', $k1);
            list($id2, $k2) = explode('-', $k2);
            preg_match_all('#中華民國(\d+)年(\d+)月(\d+)日#', $k1, $matches);
            if (!$c = count($matches[0])) {
                var_dump($k1);
                exit;
            }
            $c --;
            $v1 = sprintf("%05d%03d%02d%02d", $id1, $matches[1][$c], $matches[2][$c], $matches[3][$c]);

            preg_match_all('#中華民國(\d+)年(\d+)月(\d+)日#', $k2, $matches);
            if (!$c = count($matches[0])) {
                var_dump($k2);
                exit;
            }
            $c --;
            $v2 = sprintf("%05d%03d%02d%02d", $id2, $matches[1][$c], $matches[2][$c], $matches[3][$c]);
        ...
        $fp = fopen(__DIR__ . '/law-data/laws-versions.csv', 'w');
    ...
    {
        $laws = array();
        if (file_exists(__DIR__ . '/law-data/laws.csv')) {
        ...
        $fp = fopen(__DIR__ . '/law-data/laws.csv', 'w');
        ...
        $fp = fopen(__DIR__ . '/law-data/laws-category.csv', 'w');
    ...
    public function fetchRelate($content)
    {
        ...
        foreach ($doc->getElementsByTagName('img') as $img_dom) {
            if ($img_dom->getAttribute('src') == '/lglaw/images/relate.png') {
                ...
                if (file_exists(__DIR__ . "/law-data/laws/relate/{$id}.html")) {
                    continue;
                }
                $content = $this->http($url);
                file_put_contents(__DIR__ . "/law-data/laws/relate/{$id}.html", $content);
            }
        }
    }
    ...
    public function main()
    {
        if (!file_exists(__DIR__ . '/law-data/laws')) {
            mkdir(__DIR__ . '/law-data/laws');
            mkdir(__DIR__ . '/law-data/laws/relate');
        }
